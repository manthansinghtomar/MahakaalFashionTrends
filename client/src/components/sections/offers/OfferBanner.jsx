 ...
 * - Mobile (< lg): Limited Time Campaign Badge -> Banner Image -> Title & Description -> Discount & Validity -> CTA.
 * - Desktop (>= lg): Image on Left (col-span-5), Offer Copy & Action on Right (col-span-7).
 */
export const OfferBanner = ({ offer }) => {
  ...
  return (
    <div className="relative w-full overflow-hidden rounded-3xl bg-black text-white shadow-2xl border border-neutral-900 group">
      ...
      {/* Mobile-only Campaign Badge (Positioned ABOVE image on mobile < lg) */}
      <div className="p-6 pb-2 lg:hidden relative z-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 min-h-[480px]">
        ...
        {/* Left Column: Solid Black Background Container for Banner Image */}
        <div className="lg:col-span-5 lg:order-first relative min-h-[280px] sm:min-h-[320px] lg:min-h-full w-full overflow-hidden bg-black flex items-center justify-center p-4 sm:p-7 lg:p-8">
          {hasBannerImage ? (
            <img
              src={bannerImage.url}
        ...
        {/* Right Column: Promotion Copy and Action */}
        <div className="lg:col-span-7 lg:order-last p-6 sm:p-12 lg:p-16 flex flex-col justify-between relative z-10 space-y-6 sm:space-y-8">
          
          <div className="space-y-4">
            {/* Desktop-only Campaign Badge (Hidden on mobile < lg) */}
            <div className="hidden lg:inline-flex items-center gap-2 px-3 py-1 rounded-full bg-secondary/10 border border-secondary/20">
              <span className="w-1.5 h-1.5 rounded-full bg-secondary animate-pulse" />
              <span className="text-[10px] sm:text-xs font-semibold uppercase tracking-[0.2em] text-secondary">
                Limited Time Campaign
              </span>
            ...
            {/* Title */}
            <h3 className="text-2.5xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight text-white leading-tight">
              {title}
            </h3>
