import React from 'react';

/**
 * Status Badge for Products listing.
 * Maps status keys to localized high-contrast styled labels.
 */
export const ProductStatusBadge = ({ status = 'active' }) => {
  const normStatus = status.toLowerCase();

  let styles = 'bg-neutral-100 text-neutral-600 border-neutral-200';
  let label = status;

  if (normStatus === 'active') {
    styles = 'bg-emerald-50 text-emerald-700 border-emerald-200';
    label = 'Active';
  } else if (normStatus === 'outofstock') {
    styles = 'bg-amber-50 text-amber-700 border-amber-200';
    label = 'Out of Stock';
  } else if (normStatus === 'inactive' || normStatus === 'draft') {
    styles = 'bg-neutral-50 text-neutral-500 border-neutral-200';
    label = 'Inactive';
  }

  return (
    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold tracking-wider uppercase border ${styles}`}>
      {label}
    </span>
  );
};

export default ProductStatusBadge;
